...
import { getReviews } from '../../Redux/reviews/reviewsSelectors';
...
<p className="text-gray-700 text-xs md:text-md font-semibold">
              <span className="text-gray-700">{formattedDate(el.createdAt)}</span>
            </p>
            </div>
            </div>
          </div>
          ...
          <p className='text-black text-sm md:text-md smOnly:leading-5  font-bold mb-2 '>{el.title}</p>
      ...
      <div className="flex justify-center mt-6 lg:mt-8 gap-4">
              {visibleCount < reviewsArr.length && reviews.length > visibleCount && (
